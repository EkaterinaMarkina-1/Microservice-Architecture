...
class PaginationMeta(BaseModel):
    ...
    page: int = Field(..., ge=1,
                      description="Номер текущей страницы", example=1)
    per_page: int = Field(..., ge=1, le=100,
                          description="Количество элементов на странице", example=20)
    total_items: int = Field(..., ge=0,
                             description="Общее количество элементов", example=100)
...
class DeleteResponse(BaseModel):
    detail: str = Field(..., description="Сообщение об успешном удалении",
